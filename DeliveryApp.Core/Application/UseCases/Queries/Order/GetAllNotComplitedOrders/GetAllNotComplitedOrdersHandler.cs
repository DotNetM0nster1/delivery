...
namespace DeliveryApp.Core.Application.UseCases.Queries.Order.GetAllNotComplitedOrders
{
    public sealed class GetAllNotComplitedOrdersHandler(IAllActiveOrdersResult allActiveOrdersQuery) : IRequestHandler<GetAllNotComplitedOrdersRequest, List<OrderDto>>
    {
        ...
        public async Task<List<OrderDto>> Handle(GetAllNotComplitedOrdersRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                throw new ArgumentNullException(nameof(request.Orders));
            }
